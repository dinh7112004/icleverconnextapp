...
const READ_IDS_KEY = 'read_notification_ids'; // Lưu ID đã đọc cục bộ
...
export default function NewsScreen({ route, navigation }: any) {
    ...
    const fetchNotifications = async (showLoading = true) => {
        try {
            ...
            // Load danh sách ID đã đọc cục bộ
            ...
            const mapped = rawList.map((n: any) => {
                ...
                return {
                    ...
                    // Ó ưu tiên local read state — nếu đã đọc cục bộ thì không hiện dấu đỏ
                    unread: n.status === 'unread' && !localReadIds.includes(nId),
                    priority: n.priority,
                    data: n.data,
                    rawType: n.type
                };
            ...
            // Emit event để Header cập nhật lại badge thông báo
            DeviceEventEmitter.emit('refresh_unread_status');
        } catch (error) {
    ...
    const handleMarkAsRead = async (id: string) => {
        ...
        setNotifications(prev => {
            ...
            // Cập nhật cache để giữ trạng thái sau khi reload
            AsyncStorage.setItem('notifications_cache', JSON.stringify(updated));
            return updated;
        });

        // 2. Lưu ID đã đọc vào local storage (để override server data sau này)
        try {
            ...
            if (!readIds.includes(id)) {
                ...
                // Emit event ngay sau khi đánh dấu đã đọc
                DeviceEventEmitter.emit('refresh_unread_status');
            }
        } catch (e) {}

        // 3. Gọi API để sync với server
        try {
                    ...
                    {loading ? (
                        ...
                        <ScrollView style={{ flex: 1, paddingHorizontal: 16 }} keyboardShouldPersistTaps="handled">
                            ...
                            {officialDocuments.length === 0 ? (
                                ...
                                <TouchableOpacity
                                    key={item.id}
                                    style={[styles.mailCard, { backgroundColor: theme.surface, borderColor: theme.border }]}
                                    onPress={() => {
